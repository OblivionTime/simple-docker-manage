...
export default {
  ...
  created() {
    ...
    if (this.IsAdmin) {
      ...
      this.socketURI = "ws://" + prefix + process.env.VUE_APP_BASE_WS_API + "container/shell?name=" + this.name + "&user=root&hostname=" + this.$store.getters.hostname
    } else {
      this.socketURI = "ws://" + prefix + process.env.VUE_APP_BASE_WS_API + "container/shell?name=" + this.name + "&hostname=" + this.$store.getters.hostname
